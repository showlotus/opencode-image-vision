import { isImagePart, extractBase64, computeTimeoutBySize } from '../shared/image-utils.js'
import { extractModelFromMessages } from '../shared/model-detect.js'
...
export function createTransformHook(deps) {
  ...
  return async (input, output) => {
    try {
      ...
      // 门控：用 providers 精确数据判断模型是否支持图片，支持时跳过 parts 扫描
      const modelId = extractModelFromMessages(output.messages)
      const providers = getProviders ? await getProviders() : []
      const intervene = shouldInterveneForModel(providers, modelId)
      dbg(() => ({ event: 'gate', modelId, providersCount: providers.length, imageSupport: modelSupportsImage(providers, modelId), intervene }))
      if (!intervene) return
      ...
      const baseTimeout = provider.timeout || 120000
      ...
      try {
        ...
        for (let n = 0; n < targets.length; n++) {
          ...
          try {
            provider.timeout = computeTimeoutBySize(base64.length, baseTimeout)
      ...
      if (failCount > 0) {
        toastFail(client, total > 1 ? `${failCount}/${total} failed` : undefined, elapsedMs)
      } else if (successCount > 0) {
        toastDone(client, successCount, elapsedMs)
      }
      dbg(() => ({ event: 'hook_complete', successCount, failCount, totalMs: elapsedMs }))
    } catch (e) {

...
| 任务 | 位置 | 备注 |
|------|------|------|
| 新增/修改 hook | `plugin/index.js:43-71` | server 返回的 hook 映射对象 |
| 图片替换逻辑 | `plugin/transform.js:60-95` | base64→临时文件→指令文本 |
| 工具执行逻辑 | `plugin/analyze-image.js:25-181` | readFileSync→子 session→vision model |
| 添加新的不支持的模型 | `shared/model-detect.js:1-9` | `DEFAULT_UNSUPPORTED_MODELS` 数组 |
| 调试开关 | `shared/debug.js:5-14` | 环境变量 `IMAGE_VISION_DEBUG=1`，日志路径 `tmpdir()/iv-debug.log`，可用 `IMAGE_VISION_DEBUG_PATH` 覆盖 |
| 临时文件路径 | `shared/temp-file.js:5` | `join(tmpdir(), 'iv-images')` |
| 超时策略调整 | `shared/image-utils.js:21-25` | 每 0.5MB +20s，上限 300s |
...
| `createTransformHook` | function | `plugin/transform.js:15` | 1 (index.js:60) | 图片→临时文件→指令文本替换 |
| `createAnalyzeImageTool` | function | `plugin/analyze-image.js:25` | 1 (index.js:65) | 工具工厂：读盘→子 session→视觉模型 |
| `saveImageToTempDir` | function | `shared/temp-file.js:16` | 1 (transform.js:84) | base64 写入 `tmpdir/iv-images/<hash>.<ext>` |
| `fetchOpencodeProviders` | function | `shared/opencode.js:5` | 1 (index.js:33) | SDK providers 查询，带 5s 超时兜底 |
| `modelSupportsImage` | function | `shared/opencode.js:21` | 1 (transform.js:47) | 检查 capabilities.input.image |
| `isUnsupportedModel` | function | `shared/model-detect.js:11` | 1 (transform.js:43) | 快速黑名单匹配，零 IPC |
| `extractModelFromMessages` | function | `shared/model-detect.js:17` | 1 (transform.js:22) | 从 message.info 提取 modelId |
| `isImagePart` | function | `shared/image-utils.js:6` | 2 (index.js:45, transform.js:37) | `type==='file' && mime.startsWith('image/')` |
| `extractBase64` | function | `shared/image-utils.js:10` | 1 (transform.js:73) | 从 data URL 提取 base64 |
| `computeTimeoutBySize` | function | `shared/image-utils.js:21` | 1 (analyze-image.js:105) | 动态超时，120s 基础 + 每 0.5MB 20s |
| `createCache` | function | `shared/cache.js:1` | 1 (index.js:28) | Map 包装，内存级，重启丢失 |
| `dbg` | function | `shared/debug.js:19` | 4+ | 惰性求值日志，关闭时零开销 |
| `setDebug` | function | `shared/debug.js:12` | 1 (index.js:18) | 环境变量优先级最高 |
...
- **禁止用 zod 直接 import** — 参数 schema 必须用 SDK 的 `tool.schema.string()`（`analyze-image.js:31`），版本兼容
...
- **同步 I/O 已知热点**（性能关注项，非禁止）：`readFileSync`(analyze-image:54)、`writeFileSync`(temp-file:20)、`mkdirSync`(temp-file:17)、`appendFileSync`(debug:23) — 常规路径应避免，工具执行路径可接受

## UNIQUE STYLES

- **惰性调试日志**：`dbg()` 接受对象或函数；函数形式仅在开关开启时求值，避免关闭时仍构造参数对象（`debug.js:19-25`）
- **双级模型门控**：先快速匹配黑名单（零 IPC），未知模型才查 SDK providers（`transform.js:43-49`）
- **递归防护**：`modelId === visionModel.modelID` 时直接 return，防止视觉模型自身触发 transform（`transform.js:25`）
- **file_path 指令注入**：图片替换为带真实路径的中文指令文本，引导模型自发调用工具（`transform.js:90-93`）
...
# 调试
IMAGE_VISION_DEBUG=1  # 日志写入 tmpdir()/iv-debug.log
# 查看实际路径（各平台不同）：
node -e "console.log(require('os').tmpdir())"
# macOS 返回 /var/folders/...，Linux 返回 /tmp，Windows 返回 %LOCALAPPDATA%\Temp
# 或用 IMAGE_VISION_DEBUG_PATH 环境变量指定固定路径
...
- **state 跨 hook 共享** — `plugin/index.js:35-39` 的 `state` 对象被 4 个 hook 读写，当前顺序调用无竞态，但无锁保护。
